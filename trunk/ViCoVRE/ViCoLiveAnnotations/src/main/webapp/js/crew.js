...
function crew_add_annotation(contentid,type,refersto,modifies){
    ...
    for (var field in liveAnnotations[type].fields) {
        if (liveAnnotations[type].fields[field]["type"] == "text"){
            useboxes=true;
            textboxes+=boxsep;
            textboxes+=liveAnnotations[type].fields[field]["displayname"]+':</td><td><input type="text" id="'+field+'" ';
            if (annotation!=null) {
        ...
        if (liveAnnotations[type].fields[field]["type"] == "textarea"){
            useareas=true;
            textareas+=areasep;
            textareas += liveAnnotations[type].fields[field]["displayname"]+ROW_SEP;
            textareas += '<textarea id="'+field+'">'
            if (annotation!=null) {
                value=annotation[field];
                if (value!=null)
                    textareas += value;
        ...
        if (liveAnnotations[type].fields[field]["type"]=="messageId"){
            content+='<input type="hidden" name="'+field+'" id="'+field+'" value="'+refersto+'" />';
        }
    }
...
function crew_submit_annotation(contentid,type,time){
    ...
    for (var field in liveAnnotations[type].fields){
        inputvalue = document.getElementById(field);
        if (inputvalue && (inputvalue.value!="")){
            post_text +="&"+encodeURIComponent(field)+"="+encodeURIComponent(inputvalue.value);
        }
    }
...
function format_annotation(contentid,colour,date,author,annotation,text,modifies,allowedit){
    ...
    if (allowedit){
        ...
        for (var field in liveAnnotations[type].fields){
            value=annotation.getElementsByTagName(field);
            if (value){
                var val="";
                if (value.length>0){
                    val=value[0].firstChild.data;
                }
                localAnnotations[modifies].variables[field]=val;
            }
        }
